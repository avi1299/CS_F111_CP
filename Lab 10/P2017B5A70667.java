...
class Dealer implements Runnable {
	...
	public void run() {
		...
		synchronized(gameData.lock1)
		{
			...
			if(gameData.playerSuccessFlag[0]==true){ 
			...
			if(gameData.playerSuccessFlag[1]==true){ 
				// UNCOMMENT THE ONE LINE OF CODE WRITTEN BELOW AFTER SPECIFYING THE CONDITION
	...
	public void run() {
		...
		{			
			...
			while(gameData.gameCompleteFlag==false){
				while((gameData.noAnnouncedFlag==false)||(gameData.playerChanceFlag[this.id] == true)){
					try{
						gameData.lock1.wait();
					}catch (InterruptedException e){
						System.out.println(e);
				...
				if(!gameData.gameCompleteFlag) {	
					...
					for(int i=0;i<this.MAXNO;i++){
						if (this.ticket[i]==gameData.announcedNumber){
							this.totalNumbersFound+=1;
					...
					// and set the back ground color of the button to GREEN using the following statement
					 //this.btnOnTicket[i].setBackground(Color.GREEN)
					if(this.totalNumbersFound==this.MAXNO)
